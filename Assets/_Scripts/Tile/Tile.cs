...
public class Tile : MonoBehaviour, IPointerDownHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    [SerializeField] protected Plant placedResource;
    ...
    protected void SpawnResource(PlantType plantType,Tile tile)
    {
        if(!placedResource)
        {
            EnergyManager.Instance.ConsumeEnergy();
            PlantSpawner.Instance.SpawnPlant(plantType, transform.position, tile);
            placedResource = GetComponentInChildren<Plant>();
        }
    }
